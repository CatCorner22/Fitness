...
Goal: ${profile.goal} (${food.goalTitle}: ${food.calories} kcal, ${food.protein} g protein, ${food.carbs} g carbs, ${food.fat} g fat; TDEE ${food.tdee ?? "unknown"} ${food.surplus >= 0 ? "+" : ""}${food.surplus} kcal). Fitness check: ${profile.fitnessTier ?? "none"}. Injuries: ${profile.injuries.join(", ") || "none"}.
